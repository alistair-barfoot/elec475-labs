...
def test_single_model(model_path, labels, args):
    ...
    for _, row in labels.iterrows():
        ...
        image_path = os.path.join('images-original/images', filename)
        img = cv2.imread(image_path)
...
def main():
    ...
    parser.add_argument('-t', '--type', type=str, default='any', choices=['alex', 'vgg', 'snout', 'any'],
               help="Model type filter to use when testing multiple files: 'alex', 'vgg', 'snout' or 'any'")
    ...
    if args.all:
        ...
        print(f"Found {len(model_files)} model files matching type '{args.type}'. Testing...\n")
    else:
